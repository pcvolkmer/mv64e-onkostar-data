...
public class OncoDiagnoseMapper extends ConditionMapper<MtbDiagnosis> {
  ...
  protected String getId(MtbDiagnosis item) {
    return String.format("%s_oncodiagnose", item.getId());
  }
  ...
  public Condition map(MtbDiagnosis diagnose) {
    ...
    condition.setMeta(
        new Meta()
            ...
            .addProfile(
                "https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-onko-diagnose-primaertumor"));
    ...
    if (null != diagnose.getTopography()) {
      condition.addBodySite(
          new CodeableConcept()
              .addCoding(
                  new Coding()
                      .setCode(diagnose.getTopography().getCode())
                      .setSystem("http://terminology.hl7.org/CodeSystem/icd-o-3")
                      .setDisplay(diagnose.getTopography().getDisplay())));
    }
