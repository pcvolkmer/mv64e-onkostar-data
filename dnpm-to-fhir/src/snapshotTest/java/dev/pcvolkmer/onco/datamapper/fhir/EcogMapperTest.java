...
class EcogMapperTest extends DnpmToFhirTest {

  @ParameterizedTest
  @ValueSource(strings = {"performancestatus.json"})
  void shouldMapDiagnose(String filename) throws IOException {
    ...
    final var mapper = new EcogMapper();

    var fhir = mtb.getPerformanceStatus().stream().map(mapper::map).collect(Collectors.toList());
