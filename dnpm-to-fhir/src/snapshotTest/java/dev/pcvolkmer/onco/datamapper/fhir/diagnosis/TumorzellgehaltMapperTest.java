...
class TumorzellgehaltMapperTest extends DnpmToFhirTest {
  ...
  void shouldMapCnv(String filename) throws IOException {
    ...
    final var mapper = new TumorzellgehaltMapper();

    var fhir = mtb.getHistologyReports().stream().map(mapper::map).collect(Collectors.toList());
